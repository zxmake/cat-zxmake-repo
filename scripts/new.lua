...
function generate_package(reponame, get_data)
    ...
    if homepage then file:print('    set_homepage("%s")', homepage) end
    ...
    if type(data.licenseInfo) == "table" and data.licenseInfo.key then
        ...
        if license then file:print('    set_license("%s")', license) end
    end
    file:print("")
    ...
    if #build_system_detected > 0 then
        table.sort(build_system_detected,
                   function(a, b) return a.priority < b.priority end)
        build_system = build_system_detected[1]
    end
    if not build_system then build_system = build_systems["xmake.lua"] end
    ...
    if build_system then file:print(build_system.install(configs, package)) end
    file:print('    end)')
